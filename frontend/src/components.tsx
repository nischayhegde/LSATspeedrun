      ...
      <section className="evidence-panel" ref={evidencePanelRef} tabIndex={-1} aria-label={`Evidence file ${item.position + 1} of ${session.total_items}`}>
        ...
        {!result && (
          <label className="reasoning-box">
            ...
            <small id={`reasoning-help-${item.id}`} className={item.requires_reasoning && reasoning.trim().length < 20 ? 'reasoning-min-warn' : ''}>
              {item.requires_reasoning && reasoning.trim().length < 20
                ? `Write at least 20 characters to file (${reasoning.trim().length}/20).`
                : `${reasoning.length}/4000${item.requires_reasoning ? ' · Minimum met' : ''} · TrueFoundry grades reasoning separately; your text is stored as evidence, never as an instruction.`}
            </small>
          </label>
        )}
