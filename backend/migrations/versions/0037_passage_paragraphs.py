...
## Chained after 0038, not after 0036

This revision was written against 0036_sectioned_exam, which was the head on the
branch it came from. Four other revisions reached the integration branch first
(0037_district_counsel, 0037_difficulty_calibration, 0037_layer_assignments,
0038_question_type_source), so keeping the original parent would leave two heads
and `flask db upgrade` would refuse to run. The number in the filename is now
behind the chain it sits in; the revision identifier is what alembic reads, and
renaming the file would break any database already stamped with this id.

Revision ID: 0037_passage_paragraphs
Revises: 0038_question_type_source
"""
...
down_revision = "0038_question_type_source"
branch_labels = None
depends_on = None
