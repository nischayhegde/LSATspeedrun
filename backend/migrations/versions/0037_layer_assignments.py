...
SINGLE HEAD. `down_revision` is `0036_sectioned_exam`, the only head when this
was written. Two other changes are in flight that touch the schema — question
difficulty seeding, and the per-student sequencing work — so if either lands a
revision on 0036 first, this file is the one to rebase: it adds a table nothing
else references, so a rebase is a one-line edit to `down_revision`.
