...
def test_the_uniform_arm_really_stops_consulting_the_record(app):
    ...
    The student here has one clear leader. Pinned to ranked, every draw is that
    leader. Pinned to uniform, the draws have to spread across the candidates —
    if they did not, the comparison would fill up, report no difference, and
    read as evidence that ranking the candidates does not help.
    """
    with app.app_context():
        ...
        ranked = {trial["key"] for trial in _draws(user, question, 40)}

        app.config["ADAPTIVE_LAYERS"] = {"strategy_selection": {"holdback": 1.0}}
        uniform = {trial["key"] for trial in _draws(user, question, 40)}

        # The ranked arm keeps a deliberate exploration share, so it is not
        # required to be a single key — only to be narrower than a coin over
        # every candidate.
        assert len(uniform) == len(candidates)
        assert len(ranked) < len(uniform)
