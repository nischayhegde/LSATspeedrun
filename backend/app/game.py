...
# The whole catalog is the core career, rather than a handful of office gates.
# A focused player should see one meaningful cash purchase every 2--4 rewarded
# cases, with a gentle increase in cadence as the firm reaches larger markets.
# After the intentionally fast one-case onboarding desk, buying the 93 assets
# and 14 later offices takes 326 strong cases, inside the 180--360 case /
# three-to-four-week product target. Daily rewards,
# quests, and passive income are optional accelerators and are deliberately not
# required by the baseline model.
PROGRESSION_CASE_TARGET_BY_TIER = tuple(
    2 if tier <= 3 else 3 if tier <= 8 else 4
    for tier in range(15)
)
CORE_COMPLETION_CASE_RANGE = (180, 360)
PROGRESSION_SCORE_MULTIPLIER = 1.20
...
def _progression_asset_order() -> tuple[tuple[str, ...], ...]:
    """Return one stable, prerequisite-safe purchase order for every tier."""
    catalog_index = {item["key"]: index for index, item in enumerate(ASSETS)}
    owned: set[str] = set()
    ordered_tiers: list[tuple[str, ...]] = []
    for tier in range(len(FIRM_TIERS)):
        pending = {item["key"] for item in ASSETS if item["tier"] == tier}
        ordered: list[str] = []
        while pending:
            available = [
                item for item in ASSETS
                if item["key"] in pending
                and all(requirement in owned for requirement in item.get("requires", ()))
            ]
            if not available:
                raise RuntimeError(f"Catalog has an unreachable prerequisite at tier {tier}: {sorted(pending)}")
            item = min(
                available,
                key=lambda value: (
                    value["cost"],
                    catalog_index[value["key"]],
                ),
            )
            ordered.append(item["key"])
            owned.add(item["key"])
            pending.remove(item["key"])
        ordered_tiers.append(tuple(ordered))
    return tuple(ordered_tiers)


def _pacing_client(tier: int, owned: set[str]) -> dict:
    """Best presently reachable contract used by the committed-player model."""
    return max(
        (
            client for client in CLIENTS
            if client["tier"] <= tier
            and all(requirement in owned for requirement in client.get("requires", ()))
        ),
        key=lambda client: (client["base_fee"] * client.get("payout_mult", 1), client["base_fee"]),
    )


def _pacing_case_payout(tier: int, owned: set[str], state: dict, asset_by_key: dict[str, dict]) -> int:
    """Simulate one validated, Good-reasoning case without optional accelerators.

    The calculation mirrors the cash-bearing portions of ``settle_attempt``:
    active-case payout, validated streak, staff flat bonus, and contract close.
    It excludes quests, daily claims, rival discounts, and passive income so the
    curriculum can be completed through active question practice alone.
    """
    client = _pacing_client(tier, owned)
    if state.get("client_key") != client["key"]:
        state["client_key"] = client["key"]
        state["cases_remaining"] = client["length"]

    firm_multiplier = 1 + tier * .06 + sum(
        float(asset_by_key[key].get("payout_mult", 0)) for key in owned
    )
    firm_multiplier *= float(client.get("payout_mult", 1))
    core_payout = round(client["base_fee"] * PROGRESSION_SCORE_MULTIPLIER * firm_multiplier)
    staff_bonus = round(
        sum(int(asset_by_key[key].get("staff_flat", 0)) for key in owned)
        * PROGRESSION_SCORE_MULTIPLIER
    )

    state["streak"] += 1
    streak_cap = .20 + sum(
        float(asset_by_key[key].get("streak_bonus_cap", 0)) for key in owned
    )
    streak_bonus = round(core_payout * min(streak_cap, state["streak"] * .02))

    state["cases_remaining"] -= 1
    contract_bonus = 0
    if state["cases_remaining"] == 0:
        contract_multiplier = (
            2
            + float(client.get("contract_bonus_mult", 0))
            + sum(float(asset_by_key[key].get("contract_bonus_mult", 0)) for key in owned)
        )
        contract_bonus = round(client["base_fee"] * contract_multiplier)
        state["cases_remaining"] = client["length"]
    return max(1, core_payout + staff_bonus + streak_bonus + contract_bonus)


def _nice_price_between(minimum: int, maximum: int) -> int:
    """Choose a storefront-friendly price without changing the case count."""
    midpoint = (minimum + maximum) // 2
    magnitude = max(1, 10 ** max(0, len(str(max(1, midpoint))) - 2))
    rounded = round(midpoint / magnitude) * magnitude
    if rounded < minimum:
        rounded = math.ceil(minimum / magnitude) * magnitude
    if rounded > maximum:
        rounded = math.floor(maximum / magnitude) * magnitude
    return int(rounded if minimum <= rounded <= maximum else maximum)


PROGRESSION_ASSET_KEYS_BY_TIER = _progression_asset_order()


def _apply_catalog_progression_prices() -> None:
    """Price the authored catalog against the deterministic practice model.

    Keeping the rule here makes future content additions self-balancing: every
    asset remains a real cash purchase and additions inherit the same pacing
    without maintaining a second currency or a disconnected level gate.
    """
    asset_by_key = {item["key"]: item for item in ASSETS}

    owned: set[str] = set()
    cash = STARTING_CASH
    state = {"streak": 0, "client_key": None, "cases_remaining": 0}

    def price_next(definition: dict, tier: int) -> None:
        nonlocal cash
        target = PROGRESSION_CASE_TARGET_BY_TIER[tier]
        payouts = [
            _pacing_case_payout(tier, owned, state, asset_by_key)
            for _ in range(target)
        ]
        minimum = cash + sum(payouts[:-1]) + 1
        maximum = cash + sum(payouts)
        definition["cost"] = _nice_price_between(minimum, maximum)
        definition["pacing_cases"] = target
        cash = maximum - definition["cost"]

    for tier, asset_keys in enumerate(PROGRESSION_ASSET_KEYS_BY_TIER):
        for asset_key in asset_keys:
            if asset_key == "repaired_desk":
                # Preserve the one-case onboarding purchase and its established
                # $350 opening price. The 2--4 case cadence begins after this
                # first visible improvement to the shack.
                cash += _pacing_case_payout(tier, owned, state, asset_by_key)
                asset_by_key[asset_key]["pacing_cases"] = 1
                cash -= asset_by_key[asset_key]["cost"]
            else:
                price_next(asset_by_key[asset_key], tier)
            owned.add(asset_key)
        if tier < len(FIRM_TIERS) - 1:
            price_next(FIRM_TIERS[tier + 1], tier)


_apply_catalog_progression_prices()

ASSET_BY_KEY = {item["key"]: item for item in ASSETS}
CLIENT_BY_KEY = {item["key"]: item for item in CLIENTS}


def simulate_catalog_progression() -> dict:
    """Deterministically audit the cash-only path through the full catalog."""
    owned: set[str] = set()
    cash = STARTING_CASH
    state = {"streak": 0, "client_key": None, "cases_remaining": 0}
    purchases: list[dict] = []

    def afford(definition: dict, tier: int, kind: str) -> None:
        nonlocal cash
        cases = 0
        while cash < definition["cost"]:
            cash += _pacing_case_payout(tier, owned, state, ASSET_BY_KEY)
            cases += 1
            if cases > 20:
                raise RuntimeError(f"Progression price is out of bounds: {definition['name']}")
        cash -= definition["cost"]
        purchases.append({
            "kind": kind,
            "key": definition.get("key", f"office_{definition['tier']}"),
            "tier": tier,
            "cases": cases,
    ...
    for tier, asset_keys in enumerate(PROGRESSION_ASSET_KEYS_BY_TIER):
        for asset_key in asset_keys:
            afford(ASSET_BY_KEY[asset_key], tier, "asset")
            owned.add(asset_key)
        if tier < len(FIRM_TIERS) - 1:
            afford(FIRM_TIERS[tier + 1], tier, "office")

    cases_by_tier = [
        sum(purchase["cases"] for purchase in purchases if purchase["tier"] == tier)
        for tier in range(len(FIRM_TIERS))
    ]
    return {
        "total_cases": sum(purchase["cases"] for purchase in purchases),
        "purchase_count": len(purchases),
        "asset_count": len(owned),
        "purchases": purchases,
...
def _next_milestone(profile: PlayerProfile, owned: set[str]) -> dict | None:
    # Recommend the complete authored career in a stable order. This prevents a
    # cheap side item from displacing the next legible step and makes the
    # Command Deck's cash estimate consistent from session to session.
    for tier in range(profile.office_tier + 1):
        for asset_key in PROGRESSION_ASSET_KEYS_BY_TIER[tier]:
...
def _progression_guidance(profile: PlayerProfile, owned: set[str]) -> dict | None:
    """Estimate rewarded cases to the next recommended investment."""
    milestone = _next_milestone(profile, owned)
    if not milestone:
        return None
    ...
    score_multiplier = 1.20  # Correct answer with Good reasoning.
    firm_multiplier, staff_bonus = _firm_bonuses(profile, owned, score_multiplier)
    firm_multiplier *= float(client.get("payout_mult", 1))
    estimated_case_fee = max(1, round(client["base_fee"] * score_multiplier * firm_multiplier + staff_bonus))
    cash_remaining = max(0, int(milestone["cost"]) - profile.cash)
    return {
        "kind": milestone["kind"],
        "name": milestone["name"],
        "cost": int(milestone["cost"]),
        "cash_remaining": cash_remaining,
        "estimated_case_fee": estimated_case_fee,
        "cases_to_afford": math.ceil(cash_remaining / estimated_case_fee),
    }
...
def serialize_game(profile: PlayerProfile, include_catalog: bool = True) -> dict:
    ...
    payload = {
        ...
        "progression_guidance": _progression_guidance(profile, owned),
        "story": story,
    }
    if include_catalog:
        payload["catalog"] = {
