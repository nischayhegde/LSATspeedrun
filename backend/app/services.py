...
def select_boss_questions(user: User, count: int = 5) -> list[Question]:
    ...
    def boss_priority(question: Question) -> tuple[float, int]:
        ...
        weakness = 0.25
        if stat and stat.attempts:
            # Prefer measured weaknesses over unseen skills. Difficulty then
            # breaks ties within the same observed need band.
            weakness = (
                0.5
                + (1 - stat.correct / stat.attempts)
                + min(0.25, stat.recent_mistakes * 0.05)
            )
...
def create_study_session(user: User, mode: str) -> StudySession:
    ...
    if mode == "diagnostic":
        ...
        target_minutes = 20
        fallback_questions = select_review_questions(user)
        candidates = list(fallback_questions)
        blueprint = "cold-cases-sequence-v1"
    elif mode == "boss":
        target_minutes = 15
        boss_ranked = select_boss_questions(user, len(eligible))
        fallback_questions = boss_ranked[:5]
        candidates = _bounded_candidate_pool(fallback_questions, boss_ranked)
        blueprint = f"quill-boss-sequence-chapter-{story.chapter}"
    elif mode == "daily":
        target_minutes = user.target_minutes
