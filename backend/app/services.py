...
class SequencingProfile(NamedTuple):
    ...
    **Where question difficulty would go.** Every question in this bank is
    difficulty 3 and nothing in the adaptive path reads the field, so there is
    no difficulty signal to put here yet. When there is one, this is where it
    belongs: a `target_difficulty` beside these, derived from the same accuracy
    evidence, read by `select_random_questions` and by the passage choice in
    `select_reading_comprehension_case` to bias *which* questions a run draws
    rather than how many of each kind. That is deliberately left open. It is
    also why the two selection functions take their inputs as arguments instead
    of reaching for the profile themselves: adding a field here should not mean
    rewriting them.
    """
