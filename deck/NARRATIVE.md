...
**Deck:** 24 slides · **Runtime:** 4:49 · **Spoken script:** 758 words · **Live demo budget:** 1:14 across six slides (a seventh is a still)
...
**This click path previously said (B). It was wrong, and a wrong letter on a projector in front of a room of lawyers-to-be is the single most expensive error in the deck** — it costs the credibility of every sourced number that came before it. Corrected here and in the registry.

**Click path (Context A), 20s.** Alan never types, and **this slide does not submit.**
1. **0:00–0:04** — Point at the strategy brief. Say its name: **Prephrase Before Choices**. **Do not read its three steps aloud.**
2. **0:04–0:09** — Drag-highlight exactly one clause in the stimulus with the scratchpad markup. One drag only — let the markup be seen being made.
3. **0:09–0:12** — Select **(C)**. Do not read the other four choices.
4. **0:12–0:18** — Scroll the pre-filled reasoning into view. Read only its **first clause** aloud. **Do not type.**
5. **0:18–0:20** — Click confidence **4**. **Do not submit** — advance the slide.

**Why it stops short of Submit.** Submitting creates a fresh attempt and puts a 20–40 second live model call on stage, which at a 4:46 runtime is an eighth of the talk spent watching a spinner. The graded verdict is already waiting on the next slide, generated by the real coaching model ahead of time and stored. The script therefore ends on *"then confidence"* and contains no word that implies a submission is coming.

**Skip entirely:** the passage tab switcher, the per-question timer explanation, the client and fee line, reading any other answer choice, and the settlement numbers.

**Speaker notes (Alan, 53 words).** Real LSAT questions from released exams. Nothing here is watching. This one's an Assumption question, and the method up top is Prephrase Before Choices. I mark the stimulus, pick (C), and then the box nobody else makes you do: my reasoning, in my own words, before I'm allowed to see anything. Then confidence.

*"Nothing here is watching" is four words and it is the active-versus-passive through-line cashing out. Every beat above is the student producing something — a markup, a choice, a written argument, a confidence rating — and not one of them is reception. This is the twenty seconds the room has been promised since slide 5.*

**Transition out.** Advancing reassigns the iframe to the pre-graded session, which is a real reload. **The next slide's first sentence is written to be spoken over it** — see its notes.

---

#### 14 · `demo-case-verdict-review` — Act IV · live demo · **38 seconds**
...
| 13 | `demo-case-answer` | Alan | 55 | **21** | 157 | 2:47 |
| 14 | `demo-case-verdict-review` | Alan | 45 | **17** | 159 | 3:04 |
...
**Total 4:49 — 758 spoken words at 157 wpm. Live demo total 1:14** across the six **bolded** slides, down from 3:14. Slide 22 is *italicised* rather than bolded because it is no longer live: it is a still, and the eight seconds are speech over a frozen frame.
...
**One demo beat was removed for a reason that is not time.** Slide 13 used to end on **Submit**, and it now stops at confidence-4. Submitting creates a fresh attempt and puts a 20–40 second live frontier-model call on stage — at this runtime, an eighth of the talk spent watching a spinner, with a non-trivial chance of it being longer. The graded verdict is instead already waiting on slide 14, generated by the real coaching model ahead of time and stored, so what the audience sees is genuine model output rendered from a database read. **The cost of that trade is one app reload on the 13 → 14 advance**, because the two slides address different sessions and the iframe `src` is reassigned. That seam reads as a bug when it is silent and as a transition when it is spoken over, so slide 14's script now opens with *"here's what came back"* — a sentence Alan would have said anyway, placed deliberately on the keypress rather than after the paint. Nothing in the script claims a live call is happening, and nothing should be added that does; if a judge asks directly, the answer is in that slide's notes and it is a plain yes-we-pre-ran-it with an offer to run one cold in Q&A.
